...
#define FILTER 3		//how often to update pose data - 1 is most often 
#define X_OFFSET 0
#define Y_OFFSET 10
#define Z_OFFSET 40.0
...
int* translate(Mat transVec, int* translation)
{
...
int* rotate(Mat rotVec, int* rotation)
{
...
int main( int argc, const char** argv )
{
	...
	//camera.captureInit("/home/nissanka/Documents/prog/check_cmake/Video_00004.wmv");
	...
	{
		basePoints3d[i].x = (i % boardSize.width) * squareSize + X_OFFSET;
		basePoints3d[i].y = (i / boardSize.width) * squareSize + Y_OFFSET;
	...
	while(1)
	{
		...
		if(patternfound)
		{
			...
			if (count > 1) {
				translations = translate(tvec, translations);
				rotations = rotate(rvec, rotations);
				averageTrans[0] += translations[0];
				averageTrans[1] += translations[1];
				averageTrans[2] += translations[2];
			...
			{		
				...
				sprintf(s2, "Translation:  X=%d     Y=%d     Z=%d",  
					averageTrans[0]/FILTER, averageTrans[1]/FILTER, averageTrans[2]/FILTER);
				count = 0;
		...
		putText(dispIm, s2, Point(20,I_HEIGHT - 20), 1, 1.5, RED, 2);
		putText(dispIm, s1, Point(20,I_HEIGHT - 65), 1, 1.5, GREEN, 2);
